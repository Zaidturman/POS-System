                        ...
                        <form id="purchase-return-form" action="{{ route('purchase-returns.update', $purchase_return) }}" method="POST">
                                ...
                                <div class="col-lg-4">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="status" id="status" required>
                                            <option {{ $purchase_return->status == 'Pending' ? 'selected' : '' }} value="Pending">{{ trans('public.Pending') }}</option>
                                            <option {{ $purchase_return->status == 'Shipped' ? 'selected' : '' }} value="Shipped">{{ trans('public.Shipped') }}</option>
                                            <option {{ $purchase_return->status == 'Completed' ? 'selected' : '' }} value="Completed">{{ trans('public.Completed') }}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-lg-4">
                            ...
                            <div class="mt-3">
                                <button type="submit" class="btn btn-primary">
                                    {{ trans('public.UpdatePurchaseReturn') }} <i class="bi bi-check"></i>
                                </button>
                            </div>
                        </form>
                    </div>
