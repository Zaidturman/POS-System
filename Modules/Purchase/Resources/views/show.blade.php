            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header d-flex flex-wrap align-items-center">
                        <div>
                           {{ trans('public.Reference') }} : <strong>{{ $purchase->reference }}</strong>
                        </div>
                    ...
                    <div class="card-body">
                        <div class="row mb-4">
                            ...
                            <div class="col-sm-4 mb-3 mb-md-0">
                                ...
                                <div>{{ trans('public.Invoice') }}: <strong>INV/{{ $purchase->reference }}</strong></div>
                                <div>{{ trans('public.Date') }}: {{ \Carbon\Carbon::parse($purchase->date)->format('d M, Y') }}</div>
                                <div>
                                   {{ trans('public.Status') }} : <strong>{{ $purchase->status }}</strong>
                                </div>
                                <div>
                                   {{ trans('public.PaymentStatus') }} : <strong>{{ $purchase->payment_status }}</strong>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-lg-4 col-sm-5 ml-md-auto">
                                <table class="table">
                                    <tbody>
                                    <tr>
                                        <td class="left"><strong>{{ trans('public.Discount') }} ({{ $purchase->discount_percentage }}%)</strong></td>
                                        <td class="right">{{ format_currency($purchase->discount_amount) }}</td>
                                    </tr>
                                    <tr>
                                        <td class="left"><strong>{{ trans('public.Tax') }} ({{ $purchase->tax_percentage }}%)</strong></td>
